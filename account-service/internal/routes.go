...
func SetupEndpoints() {
	...
	_router.HandleFunc("/test", wrapHandlerWithaAuth(TestFunc))
	_router.HandleFunc("/connect", ConnectToInstance)

	// Register
	_router.HandleFunc("/account", wrapHandlerWithaAuth(CreateUser)).Methods("PUT")
	...
	_router.HandleFunc("/account", wrapHandlerWithaAuth(GetUser)).Methods("GET")
	//_router.HandleFunc("/account", GetUsers).Methods("GET")
	_router.HandleFunc("/account/verify", wrapHandlerWithaAuth(Login)).Methods("POST")
